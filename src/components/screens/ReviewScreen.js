...
import { saveWeeklyReview, saveProfile } from '../../lib/db';
import { Card, Button, Input, SectionLabel, AICard, MomentumBar } from '../ui';
...
// ─── Morning Review ───────────────────────────────────────────────────────────
function MorningReview({ profile, tasks, onSave }) {
  ...
  const handleNext = async () => {
    ...
    // Generate AI game plan
    setLoading(true);
    const text = await callAI({
      messages: [{
        role: 'user',
        content: `Morning review for ${new Date().toLocaleDateString('en-US', { weekday: 'long', month: 'long', day: 'numeric' })}.
Top 3 priorities: ${answers.priorities}
Must-win: ${answers.mustWin}
Mindset going in: ${answers.mindset}
Pending tasks today: ${tasks.filter(t => !t.done).length}
...
Give me a sharp morning game plan. 3 sentences max. Direct, grounding, no fluff.`,
    ...
    setLoading(false);
    setDone(true);
    onSave({ type: 'morning', ...answers, aiGamePlan: text, date: todayKey });
  };
    ...
    return (
      <div className="fade-in">
        ...
        <div style={{ marginTop: '14px', padding: '14px 16px', background: tokens.bgCard, border: `1px solid ${tokens.border}`, borderRadius: '10px' }}>
          <div style={{ fontSize: '11px', color: tokens.textMuted, fontWeight: 700, letterSpacing: '0.08em', marginBottom: '8px' }}>YOUR PLAN</div>
        ...
        <Button onClick={() => { setStep(0); setAnswers({ priorities: '', mustWin: '', mindset: '' }); setDone(false); setAiText(''); }} variant="ghost" style={{ marginTop: '12px' }}>
          ← Redo Morning Review
        </Button>
      </div>
    );
...
function EODReview({ tasks, onSave }) {
  ...
  const steps = [
    ...
    { field: 'reflection',   label: "How did today feel? Anything worth noting?",  placeholder: "Energy, mindset, wins, frustrations..." },
  ];
  ...
  const handleNext = async () => {
    ...
    const text = await callAI({
      messages: [{
        role: 'user',
        content: `End of day review for ${new Date().toLocaleDateString('en-US', { weekday: 'long', month: 'long', day: 'numeric' })}.
Accomplished: ${answers.accomplished}
Tasks completed: ${doneTasks.length} (${doneTasks.slice(0,3).map(t => t.title).join(', ')}${doneTasks.length > 3 ? '...' : ''})
...
Give me a brief honest EOD reflection and one key focus for tomorrow. 3 sentences max.`,
    ...
    setLoading(false);
    setDone(true);
    onSave({ type: 'eod', ...answers, aiReflection: text, date: todayKey, tasksCompleted: doneTasks.length });
  };
      ...
      <div className="fade-in">
        ...
        <div style={{ marginTop: '14px', padding: '14px 16px', background: tokens.bgCard, border: `1px solid ${tokens.border}`, borderRadius: '10px' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <div style={{ fontSize: '11px', color: tokens.textMuted, fontWeight: 700, letterSpacing: '0.08em' }}>TASKS COMPLETED TODAY</div>
        ...
        <Button onClick={() => { setStep(0); setAnswers({ accomplished: '', unfinished: '', reflection: '' }); setDone(false); setAiText(''); }} variant="ghost" style={{ marginTop: '12px' }}>
          ← Redo EOD Review
        </Button>
      </div>
    );
...
function WeeklyReview({ tasks, projects }) {
  ...
  const generateInsight = async () => {
    ...
    setAiText(text || 'Solid data for the week. Reflect on what moved and what stalled.');
    setAiLoading(false);
  };
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '12px', marginBottom: '14px' }}>
        <Input label="Key Wins This Week (one per line)" value={form.wins} onChange={v => setForm(f => ({ ...f, wins: v }))} placeholder="Sent proposal&#10;Ran 4x this week" multiline rows={4} />
        <Input label="Bottlenecks & Stalls (one per line)" value={form.bottlenecks} onChange={v => setForm(f => ({ ...f, bottlenecks: v }))} placeholder="Contractor still unresolved" multiline rows={3} />
        <Input label="Personal Notes" value={form.notes} onChange={v => setForm(f => ({ ...f, notes: v }))} placeholder="Anything worth capturing..." multiline rows={3} />
      </div>
...
export default function ReviewScreen() {
  ...
  const tabs = [
    { id: 'morning', label: '☀ Morning',  subtitle: 'Plan your day' },
    { id: 'eod',     label: '🌙 End of Day', subtitle: 'Reflect & reset' },
    { id: 'weekly',  label: '◷ Weekly',   subtitle: 'Strategic review' },
  ];

  const handleSaveDailyReview = async (data) => {
    if (!user) return;
    const key = `review_${data.type}_${data.date}`;
  ...
  return (
    <div style={{ maxWidth: 720, margin: '0 auto' }}>
      <div className="fade-up" style={{ marginBottom: '24px' }}>
        ...
        <p style={{ color: tokens.textSecondary, fontSize: '13px', marginTop: '4px' }}>Morning planning · End of day reflection · Weekly strategy</p>
      </div>
      ...
      <div className="fade-up stagger-1" style={{ display: 'flex', gap: '8px', marginBottom: '24px', background: tokens.bgCard, padding: '6px', borderRadius: '12px', border: `1px solid ${tokens.border}` }}>
        {tabs.map(tab => (
          <button key={tab.id} onClick={() => setActiveTab(tab.id)}
            style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '2px', padding: '10px 8px', borderRadius: '8px', border: 'none', background: activeTab === tab.id ? tokens.accentDim : 'transparent', color: activeTab === tab.id ? tokens.accent : tokens.textSecondary, cursor: 'pointer', transition: 'all 0.15s', fontFamily: fonts.body }}>
            <span style={{ fontSize: '13px', fontWeight: activeTab === tab.id ? 700 : 400 }}>{tab.label}</span>
            <span style={{ fontSize: '10px', color: activeTab === tab.id ? tokens.accent : tokens.textMuted, opacity: 0.8 }}>{tab.subtitle}</span>
          </button>
        ))}
      </div>
      ...
      <div className="fade-up stagger-2">
        {activeTab === 'morning' && <MorningReview profile={profile} tasks={tasks} onSave={handleSaveDailyReview} />}
